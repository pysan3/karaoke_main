...
std::vector<int> monauralize(std::vector<int> &list, int data_length, int channels) {
    int i, j, ave;
    ...
    if (channels == 2) {
        ...
        for (i=0; i<data_length; ++i) {
            ave = 0;
            for (j=0; j<channels; ++j) ave += list[i*channels+j];
...
PYBIND11_PLUGIN(cpplib){
    py::module m("cpplib", "for optimized vector calculation");
